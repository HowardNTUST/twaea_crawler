...
from lib.IR_U_request import (add_year_col, get_candidate_info, getdepartment,
                              getschool_links, request_url)
...
def main_crawler(mode, year):
    ...
    # # 正式
    # all_volunteer_data = Parallel(n_jobs=30)( # url = all_department_links_df['link'].values[0]
    #                 delayed(get_candidate_info)(url,n) for n,url in enumerate(all_department_links_df['link'].values)
    #             )
